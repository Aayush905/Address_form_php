...
$tid=null;
 if($caller==1){
    $sql = "SELECT * FROM info ORDER BY {$order_by} LIMIT {$offset},{$limit_per_page}";
    $tid="up";
 }else if($caller==0){
    $sql = "SELECT * FROM info ORDER BY {$order_by} DESC LIMIT {$offset},{$limit_per_page}";
    $tid="down";
 }
  ...
  <tbody>'; 
    ...
    if($caller==1){
        $temp="SELECT * FROM info ORDER BY {$order_by}";
    }else if($caller==0){
        $temp="SELECT * FROM info ORDER BY {$order_by} DESC";
    }
